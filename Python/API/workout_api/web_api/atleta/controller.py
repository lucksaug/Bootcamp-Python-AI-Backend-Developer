...
from web_api.atleta.models import AtletaModel
from web_api.atleta.schemas import AtletaIn, AtletaOut, AtletaUpdate
from web_api.categorias.models import CategoriaModel
from web_api.centro_treinamento.models import CentroTreinamentoModel
from web_api.contrib.dependencies import DatabaseDependency
...
async def post(
    ...
    cpf = await db_session.execute(
        select(AtletaModel).filter_by(cpf=cpf_atleta).scalars().first()
    )
    if cpf:
        raise IntegrityError(f"O atleta com o CPF {cpf_atleta} já existe")
    ...
    try:
        ...
        atleta_model.categoria_id = categoria.pk
        atleta_model.centro_treinamento_id = centro_treinamento.pk
...
async def query(
    ...
    atleta_up.model_dump(exclude_unset=True)
